...
impl MintRequest {
    pub fn new(name_type: NameType) -> Self {
        Self {
            ...
            attribution: crate::attribution::Attribution::default(),
            program_pid: None,
            compartment_id: None,
        }
    }
...
mod tests {
    ...
    fn remints_on_collision() {
        ...
        // Ledger seq feeds the alpha, so a second mint from the same authority
        // naturally lands on a different name even without a forced collision.
        let mut minter = Minter::new(&auth, &pools, &linter, &mut ledger);
